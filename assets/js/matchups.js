    ...
    const ODDS_HISTORY_URL = new URL('data/odds_history.json', document.baseURI);
    ...
    function parseGameId(gameId) {
      ...
      // Expected: SEASON_WEEK_AWAY_HOME (e.g., 2025_18_BUF_MIA or 2025_WC_BUF_MIA)
      ...
      const away = parts[2];
      const home = parts[3];
      if (!Number.isFinite(season) || !Number.isFinite(week) || !away || !home) return null;
      return { season, week, teamA: away, teamB: home, away, home };
    }

    let data;
    let schedulePayload = null;
    let oddsPayload = null;
    let oddsHistoryPayload = null;
    let oddsIndex = {};
    let oddsHistoryIndex = {};
    let mode = 'raw';
    let momentumMode = urlParams.get('momentum') === '1';
    ...
    let neutralField = localStorage.getItem('neutralField') === '1';
    if (neutralToggle) {
      neutralToggle.checked = neutralField;
      neutralToggle.addEventListener('change', () => {
        neutralField = neutralToggle.checked;
        localStorage.setItem('neutralField', neutralField ? '1' : '0');
        renderMatchups();
      });
    }
    ...
    function normalizeWeekValue(weekValue, seasonNum = null, seasonType = null) {
      ...
      const base = getRegSeasonMax(seasonKey);
    ...
    function getRegSeasonMax(seasonNum = null) {
      const seasonKey = seasonNum != null ? String(seasonNum) : null;
      if (seasonKey && Number.isFinite(REG_MAX_BY_SEASON[seasonKey])) {
        return REG_MAX_BY_SEASON[seasonKey];
      }
      const seasonNumber = Number(seasonNum);
      if (!Number.isFinite(seasonNumber)) return 18;
      return seasonNumber >= 2021 ? 18 : 17;
    }

    function inferPostseasonRound(weekValue, seasonNum = null) {
      const regMax = getRegSeasonMax(seasonNum);
      const delta = Number(weekValue) - regMax;
      if (!Number.isFinite(delta) || delta < 1 || delta > POSTSEASON_ORDER.length) return null;
      return POSTSEASON_ORDER[delta - 1] || null;
    }

    function getWeekTypeMap(seasonKey) {
      if (WEEK_TYPE_CACHE[seasonKey]) return WEEK_TYPE_CACHE[seasonKey];
      const season = data?.seasons?.[seasonKey];
      if (!season) {
        WEEK_TYPE_CACHE[seasonKey] = {};
        return WEEK_TYPE_CACHE[seasonKey];
      }
      const map = {};
      (season.games || []).forEach((g) => {
        ...
        if (!type) return;
        if (POSTSEASON_ORDER.includes(type)) {
          map[week] = type;
          return;
        }
        if (type === 'POST' && !map[week]) {
          map[week] = 'POST';
        }
        if (type === 'REG' && !map[week]) {
          map[week] = 'REG';
        }
      });
      WEEK_TYPE_CACHE[seasonKey] = map;
      return map;
    }
    ...
    function formatWeekLabel(value, seasonNum = null) {
      ...
      const seasonKey = seasonNum != null ? String(seasonNum) : null;
      const typeMap = seasonKey ? getWeekTypeMap(seasonKey) : {};
      const mappedType = typeMap?.[value];
      if (mappedType && POSTSEASON_LABELS[mappedType]) return POSTSEASON_LABELS[mappedType];
      if (mappedType === 'POST') {
        const round = inferPostseasonRound(value, seasonNum);
        if (round && POSTSEASON_LABELS[round]) return POSTSEASON_LABELS[round];
      }
      const inferredRound = inferPostseasonRound(value, seasonNum);
      if (inferredRound && POSTSEASON_LABELS[inferredRound]) return POSTSEASON_LABELS[inferredRound];
      return `Week ${value}`;
    }
    ...
    async function fetchOddsHistory() {
      try {
        const oddsHistoryUrl = new URL(ODDS_HISTORY_URL);
        oddsHistoryUrl.searchParams.set('v', String(Date.now()));
        const res = await fetch(oddsHistoryUrl, { cache: 'no-store' });
        if (!res.ok) throw new Error('Failed to load odds history');
        oddsHistoryPayload = await res.json();

        oddsHistoryIndex = {};
        const rows = Array.isArray(oddsHistoryPayload?.history) ? oddsHistoryPayload.history : [];
        rows.forEach((r) => {
          const gid = r?.game_id || r?.gameId;
          if (!gid) return;
          (oddsHistoryIndex[gid] ||= []).push(r);
        });

        Object.values(oddsHistoryIndex).forEach((arr) => {
          arr.sort((a, b) => String(a?.updated_at || '').localeCompare(String(b?.updated_at || '')));
        });
      } catch (err) {
        oddsHistoryPayload = null;
        oddsHistoryIndex = {};
        console.warn('Odds history not available', err);
      }
    }
    ...
    function mergeScheduleIntoData() {
      ...
      Object.entries(scheduleSeasons).forEach(([seasonKey, sched]) => {
        ...
        (sched?.games || []).forEach((g) => {
          ...
          const seasonType = g?.season_type ?? g?.seasonType ?? null;
          const gameType = g?.game_type ?? g?.gameType ?? null;
          const postseason = g?.postseason ?? null;
          const entries = [
            { game_id: gameId, week, team: home, opp: away, season_type: seasonType, game_type: gameType, postseason },
            { game_id: gameId, week, team: away, opp: home, season_type: seasonType, game_type: gameType, postseason },
          ];
    ...
    function mergeOddsScheduleIntoData() {
      ...
      (oddsPayload.odds || []).forEach((o) => {
        ...
        const seasonType = o?.season_type ?? o?.seasonType ?? null;
        if (!seen.has(keyA)) {
          season.games.push({
            game_id: gameId,
            week: weekNum,
    ...
    function repairGamesFromScheduleAndOdds() {
      ...
      // Recompute weeks from games so the dropdown can’t "lose" weeks.
    ...
    function weightedMeanEPA(samples) {
      if (!Array.isArray(samples) || samples.length === 0) return null;
      const valid = samples.filter((s) => Number.isFinite(s.epa) && Number.isFinite(s.plays) && s.plays > 0);
      if (!valid.length) return null;
      const totalPlays = valid.reduce((acc, s) => acc + s.plays, 0);
      if (totalPlays <= 0) return null;
      const totalEPA = valid.reduce((acc, s) => acc + s.epa * s.plays, 0);
      return totalEPA / totalPlays;
    }

    function buildMomentumMap(season, seasonKey) {
      if (MOMENTUM_CACHE[seasonKey]) return MOMENTUM_CACHE[seasonKey];
      const byTeam = {};
      (season.games || []).forEach((g) => {
        const team = normalizeTeam(g.team || g.home || g.away);
        const week = normalizeWeekValue(g.week, seasonKey, g?.season_type);
        if (!team || !Number.isFinite(week)) return;
        if (!byTeam[team]) byTeam[team] = [];
        byTeam[team].push({ ...g, _week: week });
      });

      const momentum = {};
      Object.entries(byTeam).forEach(([team, games]) => {
        games.sort((a, b) => a._week - b._week);
        const offHistory = [];
        const defHistory = [];
        momentum[team] = {};
        games.forEach((game) => {
          const offSlice = offHistory.slice(-MOMENTUM_WINDOW);
          const defSlice = defHistory.slice(-MOMENTUM_WINDOW);
          momentum[team][game._week] = {
            off: weightedMeanEPA(offSlice),
            def: weightedMeanEPA(defSlice),
            offN: offSlice.length,
            defN: defSlice.length,
          };
          if (Number.isFinite(game.off_epa_pp) && Number.isFinite(game.off_plays)) {
            offHistory.push({ epa: game.off_epa_pp, plays: game.off_plays });
          }
          if (Number.isFinite(game.def_epa_pp) && Number.isFinite(game.def_plays)) {
            defHistory.push({ epa: game.def_epa_pp, plays: game.def_plays });
          }
        });
      });
    ...
    function applyMomentumRatings(baseRatings, season, week, seasonKey) {
      const momentumMap = buildMomentumMap(season, seasonKey);
      const ratings = {};
      Object.entries(baseRatings).forEach(([team, entry]) => {
        const momentum = momentumMap?.[team]?.[week] || {};
        const offEPA = Number.isFinite(momentum.off) ? momentum.off : entry.offEPA;
        const defEPA = Number.isFinite(momentum.def) ? momentum.def : entry.defEPA;
        ratings[team] = {
          ...entry,
          offEPA,
          defEPA,
    ...
    function formatSigned(val, digits = 2) {
      ...
      return `${val > 0 ? '+' : '−'}${Math.abs(Number(rounded)).toFixed(digits)}`;
    }

    function formatPoints(val, digits = 1) {
      if (!Number.isFinite(val)) return '—';
      const rounded = Number(val).toFixed(digits);
      if (Number(rounded) === 0) return '≈ 0.0 pts';
      return `≈ ${val > 0 ? '+' : '−'}${Math.abs(Number(rounded)).toFixed(digits)} pts`;
    }

    function formatPointsShort(val, digits = 1) {
      if (!Number.isFinite(val)) return '—';
      const rounded = Number(val).toFixed(digits);
      if (Number(rounded) === 0) return '≈ 0.0 pts';
      return `≈ ${val > 0 ? '+' : '−'}${Math.abs(Number(rounded)).toFixed(digits)} pts`;
    ...
    function buildEdgeBadge(lane, totalEdges, { showRankMeta = true } = {}) {
      ...
      const advantagedUnit = lane.desc.direction === 'off'
        ...
        : lane.desc.direction === 'def'
          ? `${lane.def} Def`
          : 'Even';
      const epaText = (!isNoData && Number.isFinite(lane.edge)) ? `EPA/play ${formatSigned(lane.edge)}` : null;
      ...
      const primaryLabel = isNoData ? 'NO DATA' : (isEven ? 'Even' : advantagedUnit);
      const metaParts = [primaryLabel, epaText];
    ...
    function buildNetBadge(game, gameTotal, { showRankMeta = true, showPercentile = true } = {}) {
      ...
      const valueEl = document.createElement('div');
      valueEl.className = 'value';
      const netPoints = Number.isFinite(game.netEdge) ? formatPointsShort(epaToPoints(game.netEdge)) : '—';
      valueEl.textContent = netPoints;
      ...
      const epaText = Number.isFinite(game.netEdge) ? `EPA/play ${formatSigned(game.netEdge)}` : null;
      const metaParts = [epaText, pctText, rankText].filter(Boolean);
    ...
    function buildGameDetails(game, gameTotal) {
      ...
      const lines = [
        ...
        `NET advantage (${game.winnerTeam || '—'}): ${formatSigned(game.netEdge)} (${formatPoints(epaToPoints(game.netEdge))})`,
    ...
    function buildSparkline(values, {
      ...
      const pts = series.map((row) => row.value);
      ...
      const xy = pts.map((v, i) => {
        const x = pad + (w * (i / (pts.length - 1)));
        const y = pad + (h - (h * ((v - min) / span)));
        return [x, y];
      });
      ...
      if (zeroLine && min <= 0 && max >= 0) {
        const zeroY = pad + (h - (h * ((0 - min) / span)));
      ...
      xy.forEach(([x, y], i) => {
        ...
        circle.setAttribute('r', isLast ? '3' : '2');
    ...
    function buildOddsMovementBlock(gameId, { home, away, modelHomeMargin } = {}) {
      const hist = oddsHistoryIndex?.[gameId] || [];
      if (hist.length < 2) return null;

      const labels = hist.map((r) => String(r.updated_at || ''));
      const spreads = hist.map((r) => getHomeSpreadFromOdds(r, home, away));
      const totals = hist.map((r) => Number(r.total));
      const edges = Number.isFinite(modelHomeMargin)
        ? spreads.map((s) => (Number.isFinite(s) ? modelHomeMargin + s : NaN))
        : null;

      const wrap = document.createElement('div');
      wrap.className = 'odds-movement';

      function addSparkRow(label, series, fmt, { zeroLine = false } = {}) {
        if (!Array.isArray(series) || series.filter((v) => Number.isFinite(Number(v))).length < 2) return;
      ...
      addSparkRow('Spread', spreads, (v) => fmtSigned(roundHalf(v)));
      addSparkRow('Total', totals, (v) => Number(v).toFixed(1));
      if (edges) addSparkRow('Edge', edges, (v) => fmtSigned(roundHalf(v)), { zeroLine: true });

      if (!wrap.children.length) return null;
      return wrap;
    }

    function getHomeSpreadFromOdds(odds, home, away) {
      const spread = Number(odds?.spread);
      if (!Number.isFinite(spread)) return NaN;

      const spreadTeam = normalizeTeam(odds?.spread_team);
      const homeTeam = normalizeTeam(home);
      const awayTeam = normalizeTeam(away);

      if (spreadTeam && awayTeam && homeTeam && spreadTeam === awayTeam) return -spread;
      return spread;
    }
    ...
    function renderGameCards(gameEntries, lanesByGame, edgeCount, { seasonKey, scale, gameTotal }) {
      ...
      gameEntries.forEach((game) => {
        ...
        const netSummary = game.winnerTeam
          ? `NET: ${game.winnerTeam} ${formatPoints(epaToPoints(game.netEdge))} (${formatSigned(game.netEdge)} EPA/play)`
          : 'NET: —';
        summaryText.innerHTML = `<div>${netSummary}</div>`;
        const drivers = document.createElement('div');
        drivers.className = 'drivers';
        drivers.textContent = `Components: OFF ${formatPoints(epaToPoints(game.offDriver))} (${formatSigned(game.offDriver)} EPA/play) · DEF ${formatPoints(epaToPoints(game.defDriver))} (${formatSigned(game.defDriver)} EPA/play)`;
        ...
        const totalValue = Number.isFinite(totalNum) ? totalNum.toFixed(1) : '—';
        ...
        oddsRow.innerHTML = `<span>${oddsPieces[0]}</span><span>${oddsPieces[1]}</span>`;
        if (metaText) {
          oddsRow.title = metaText;
          const info = document.createElement('span');
          info.className = 'odds-info';
          info.textContent = 'ⓘ';
          info.title = metaText;
          info.setAttribute('aria-label', metaText);
          oddsRow.appendChild(info);
        }

        const parsed = parseGameId(game.game_id);
        const home = normalizeTeam(parsed?.home);
        const away = normalizeTeam(parsed?.away);
        const netLeaderTeam = normalizeTeam(game.netTeam || game.netLeader || game.winnerTeam);
        const netPts = epaToPoints(game.netEdge);
        const netPtsAbs = Number.isFinite(netPts) ? Math.abs(netPts) : NaN;
        const marketSpread = getHomeSpreadFromOdds(odds, home, away);

        const betBox = document.createElement('div');
        let modelHomeMargin = NaN;

        if (!netLeaderTeam || !Number.isFinite(netPtsAbs) || !Number.isFinite(marketSpread) || !home || !away) {
          betBox.className = 'bet-box';
          betBox.innerHTML = `
            <div class="bet-box__header">
              <div class="bet-box__title">Model vs Spread</div>
            </div>
            <div class="bet-box__v">Unavailable (missing EPA or spread).</div>
          `;
        } else {
          const baseHomeMargin = netLeaderTeam === home ? netPtsAbs : -netPtsAbs;
          modelHomeMargin = baseHomeMargin + (neutralField ? 0 : HFA_POINTS);
          const modelHomeSpread = -modelHomeMargin;
          const modelHomeSpreadRounded = roundHalf(modelHomeSpread);
          const edge = modelHomeMargin + marketSpread;
          const edgeRounded = roundHalf(edge);
          let rightSide = 'PUSH';
          if (edge > 0) rightSide = home;
          if (edge < 0) rightSide = away;

          const edgeClass = edgeRounded > 0 ? 'bet-box--pos' : edgeRounded < 0 ? 'bet-box--neg' : '';
          betBox.className = `bet-box ${edgeClass}`;

          const accent = rightSide === 'PUSH' ? '#0f172a' : (TEAM_COLORS[normalizeTeam(rightSide)] || '#0f172a');
          betBox.style.setProperty('--accent', accent);

          const modelLine = `${home} ${fmtSigned(modelHomeSpreadRounded)}`;
          const marketLine = `${home} ${fmtSigned(roundHalf(marketSpread))}`;

          betBox.innerHTML = `
            <div class="bet-box__header">
              <div class="bet-box__title">Model vs Spread</div>
              <div class="bet-box__note">${neutralField ? 'Neutral' : '+3 HFA'}</div>
            </div>

            <div class="bet-box__grid">
              <div class="bet-box__k">Model</div>
              <div class="bet-box__v">${modelLine}</div>

              <div class="bet-box__k">Market</div>
              <div class="bet-box__v">${marketLine}</div>
            </div>

            <div class="bet-box__result">
              <div>
                <div class="bet-box__edge-k">Edge</div>
                <div class="bet-box__edge-v">${fmtSigned(edgeRounded)}</div>
              </div>

              <div>
                <div class="bet-box__pick-k">Right side</div>
                <div class="bet-box__pick-v">${rightSide}</div>
              </div>
            </div>
          `;
        }

        const bettingStrip = document.createElement('div');
        bettingStrip.className = 'betting-strip';
        bettingStrip.appendChild(oddsRow);
        const movement = buildOddsMovementBlock(game.game_id, { home, away, modelHomeMargin });
        if (movement) bettingStrip.appendChild(movement);
        bettingStrip.appendChild(betBox);
        scoreboard.appendChild(bettingStrip);

        const detailsEl = buildGameDetails(game, gameTotal);
        scoreboard.appendChild(detailsEl);
    ...
    function renderMatchups() {
      ...
      const momentumRatings = momentumMode ? applyMomentumRatings(baseRatings, season, week, seasonKey) : baseRatings;
      const ratings = mode === 'sos'
        ? computeSOSAdjustedRatings(season, Math.max(1, week), momentumRatings, seasonKey)
        : momentumRatings;
      if (noRatings) {
      ...
      const weeksLabel = weeks.length ? `${formatWeekLabel(weeks[0], seasonKey)}–${formatWeekLabel(weeks[weeks.length - 1], seasonKey)}` : 'n/a';
    ...
    async function fetchOdds() {
      try {
        ...
        schedules.forEach((sched) => {
          (sched?.games || []).forEach((g) => {
            if (!g?.game_id) return;
            scheduleMap[g.game_id] = {
              home: normalizeTeam(g?.home),
              away: normalizeTeam(g?.away),
            };
          });
        });
        (oddsPayload.odds || []).forEach((o) => {
          if (o?.spread_team) return;
          const spreadNum = Number(o?.spread);
          if (!Number.isFinite(spreadNum)) return;
          const gameId = o?.game_id || o?.gameId;
          const scheduleTeams = scheduleMap[gameId] || {};
          const parsed = parseGameId(gameId);
          const home = normalizeTeam(scheduleTeams.home || parsed?.home);
          const away = normalizeTeam(scheduleTeams.away || parsed?.away);
          if (!home || !away) return;
          o.spread_team = home;
          o.spread = -spreadNum;
        });
    ...
    async function init() {
      ...
      Object.keys(WEEK_TYPE_CACHE).forEach((key) => delete WEEK_TYPE_CACHE[key]);
      Object.keys(MOMENTUM_CACHE).forEach((key) => delete MOMENTUM_CACHE[key]);
      ...
      if (momentumToggle) momentumToggle.checked = momentumMode;
      onSeasonChange();
    }
    ...
    if (momentumToggle) {
      momentumToggle.addEventListener('change', () => {
        momentumMode = momentumToggle.checked;
        const url = new URL(window.location.href);
        if (momentumMode) {
          url.searchParams.set('momentum', '1');
        } else {
          url.searchParams.delete('momentum');
        }
